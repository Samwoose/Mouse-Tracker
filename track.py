...
def floorCrop(filename):
    ...
    #preprocessing
    h=525
    frame = (255-frame[285:1080-270,695:1920-700])
    #frame = (frame[285:1080-270,695:1920-700])
    
    
    #Check first frame
    print(f'frame1 size:{frame.shape} ')
    
    
    frameGray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    print(f'frameGray1 size:{frameGray.shape} ')
    kernelSize = (5,5)
...
def trace(filename):
    ...
    while frame is not None:
        ...
        if frame is None:   # not logical
            break
        frameColor = (255-frame[285:1080-270,695:1920-700]).copy() #####changes
        
        #debugging
        cv2.imshow('frame itself before subtract background', frame)
        
        #test without back ground
        #frame = cv2.subtract(frame, background)
        
        #debugging
        cv2.imshow('frame itself after subtract background', frame)
        
        t = cap.get(cv2.CAP_PROP_POS_MSEC) / 1000.
        frame = (255-frame[285:1080-270,695:1920-700]) ######changes
...
for filename in glob.glob('*.avi'):
    print("floor Crop Starts")
    floorCrop(filename)
    print("floor Crop Done")
for filename in glob.glob('*.avi'):
    print("trace Starts")
